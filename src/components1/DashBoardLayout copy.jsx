...
>>>>>>> d205858e5925be3f470811f69919484c4d8b25c4
...
export default function DashBoardLayout() {
  ...
  const [chatUsers, setChatUsers] = useState(["Mradul", "Aryan", "Srijan", "Emon", "Saritha", "Ankit", "Annie"]);
...
=======
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded text-black w-80">
              ...
              <div className="flex justify-end gap-2">
                <button className="px-4 py-1 bg-gray-300 rounded" onClick={() => setShowAddUserPopup(false)}>Cancel</button>
                <button className="px-4 py-1 bg-[#23b5b5] text-black rounded" onClick={handleAddUser}>Add</button>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Sidebar */}
        <aside className={`fixed z-30 md:static w-64 bg-black border-r border-b border-gray-800 flex-shrink-0 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
          <div className="flex justify-center p-4" onClick={() => navigate("/")}> <img src="/Explified_logo.png" alt="Logo" className="w-10 h-10" /> </div>

          <nav className="flex-1 overflow-y-auto p-4 space-y-3">
            {/* Tools Dropdown */}
            <div>
              <button onClick={() => setToolsDropdown(!toolsDropdown)} className="w-full flex justify-between items-center p-2 rounded hover:bg-gray-800 border border-gray-600">
>>>>>>> d205858e5925be3f470811f69919484c4d8b25c4
                <span>Tools</span>
                <ChevronDown size={16} />
              </button>
...
<<<<<<< HEAD
          ...
          <div className="px-4 pb-4 space-y-3">
            ...
            {/* Credits/Upgrade */}
=======
            <button onClick={() => { navigate("/socials"); setSidebarOpen(false); }} className="w-full text-left p-2 rounded hover:bg-gray-800 border border-gray-600">Socials</button>

            {/* Workflows Dropdown */}
            <div className="flex border border-gray-600 rounded">
              <button onClick={() => { navigate("/workflows"); setSidebarOpen(false); }} className="w-full text-left p-2 rounded flex items-center justify-between">Workflows</button>
              <div className="p-2" onClick={() => setShowWorkDropdown(!showWorkDropdown)}><ChevronDown size={16} /></div>
            </div>
            {showWorkDropdown && (
              <div className="z-50 mt-2 p-2 space-y-1 border border-gray-600 rounded">
                <button onClick={() => { navigate("/workflows"); setSidebarOpen(false); }} className="block w-full text-left px-2 py-1 rounded hover:bg-[#23b5b5] hover:text-black">Workflows 1</button>
                <button onClick={() => { navigate("/templates"); setSidebarOpen(false); }} className="block w-full text-left px-2 py-1 rounded hover:bg-[#23b5b5] hover:text-black">Workflows 2</button>
              </div>
            )}

            {/* Chats Dropdown with Add User Icon */}
            <div>
              <button onClick={() => setShowChatsDropdown(!showChatsDropdown)} className="w-full flex justify-between items-center p-2 rounded hover:bg-gray-800 border border-gray-600">
                <span>Chats</span>
                <div className="flex items-center gap-1">
                  <Plus size={16} onClick={(e) => { e.stopPropagation(); setShowAddUserPopup(true); }} className="hover:text-[#23b5b5]" />
                  <ChevronDown size={16} />
                </div>
              </button>
              {showChatsDropdown && (
                <div className="mt-2 p-2 space-y-1 border border-gray-600 rounded">
                  {chatUsers.filter(name => name !== "Srijan").map((name, index) => (
                    <button
                      key={index}
            ...
            <button onClick={() => { navigate("/favorites"); setSidebarOpen(false); }} className="w-full text-left p-2 rounded hover:bg-gray-800 border border-gray-600">Favorites</button>
          </nav>
...
=======
          <header className="flex justify-between items-center p-4 border-b border-gray-800 bg-black z-10">
            <div className="flex items-center gap-4 ml-4 space-x-4">
              <img src="/Explified_logo.png" alt="Logo" className="ml-4 w-10 h-10 md:hidden" onClick={() => navigate("/")} />
              <button className="text-lg font-semibold hidden md:block" onClick={() => navigate("/")}>Home</button>
              <button onClick={() => navigate("/history")} className="text-lg font-semibold hidden md:block">History</button>
              <button onClick={() => navigate("/integrations")} className="text-lg font-semibold hidden md:block">Integrations</button>
            </div>
            <div className="relative flex items-center gap-4">
              <button className="md:hidden text-white" onClick={() => setSidebarOpen(!sidebarOpen)}>{sidebarOpen ? <X size={24} /> : <Menu size={24} />}</button>
              <button onClick={() => setProfileDropdown(!profileDropdown)} className="flex items-center space-x-2 p-2 rounded hover:text-[#23b5b5] transition-colors">
                <User size={20} />
                <ChevronDown size={16} />
              </button>
>>>>>>> d205858e5925be3f470811f69919484c4d8b25c4
...
=======
                  ...
                  <div className="border-t border-white my-2" />
                  <button className="w-full py-2 flex justify-center items-center space-x-2"><LogOut size={16} /><span>Log Out</span></button>
>>>>>>> d205858e5925be3f470811f69919484c4d8b25c4
                  <div className="border-t border-white my-2" />
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 bg-black">
              ...
              <div className="h-[calc(100vh-200px)] overflow-y-auto space-y-2">
                {(messages[activeChat] || []).map((msg, i) => (
                  <div key={i} className={`my-1 ${msg.from === "Srijan" ? "text-right" : "text-left"}`}>
                    <span className="inline-block bg-[#23b5b5] text-black px-3 py-1 rounded">{msg.message}</span>
                  </div>
                ))}
              </div>
              <form className="mt-4 flex gap-2" onSubmit={(e) => {
                ...
                if (msg) {
                  socket.current.emit("send-message", { to: activeChat, from: "Srijan", message: msg });
                  setMessages(prev => ({
                    ...prev,
                    [activeChat]: [...(prev[activeChat] || []), { from: "Srijan", message: msg, time: Date.now() }],
                  }));
                  e.target.reset();
                }
              }}>
                <input name="input-message" className="flex-1 p-2 rounded text-black" placeholder="Type your message..." />
                <button className="bg-[#23b5b5] px-4 py-2 rounded text-black">Send</button>
              </form>
            </div>
          ) : (
            <main className="flex-1 -m-5 overflow-y-auto p-4 sm:p-6 md:p-8 bg-black">
