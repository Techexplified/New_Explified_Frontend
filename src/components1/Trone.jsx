...
function Trone({ onFirstPrompt }) {
  ...
  const formatText = (text) => {
    ...
    return text
      ...
      .replace(/```([\s\S]*?)```/g, '<pre style="background: #2a2a2a; padding: 12px; border-radius: 8px; margin: 8px 0; overflow-x: auto;"><code>$1</code></pre>')
  ...
  const handleSubmit = async (e) => {
    if (e.key === "Enter" && prompt.trim() !== "") {
      ...
      try {
        ...
        const geminiResponse = res.data.candidates?.[0]?.content?.parts?.[0]?.text || "No response received.";
  ...
  const handleInputChange = (e) => {
    ...
    if (value.length <= 2000) { // Limit input length
      setPrompt(value);
    }
  };
            ...
            <div className="flex items-center gap-4">
              <FiMic
                className={`text-lg cursor-pointer transition-colors ${
                  isRecording ? "text-red-500" : isTyping ? "text-gray-600" : "text-gray-400 hover:text-white"
                }`}
