...
const GeminiOrbEffect = ({ onClose }) => {
  ...
  const bgGradient = ctx.createRadialGradient(cx, cy, radius * 0.4, cx, cy, radius * 1.2);
  ...
  waveGradient.addColorStop(0, `rgba(35,181,181,${0.22 - i * 0.05 + (speaking ? 0.05 : 0)})`);
  waveGradient.addColorStop(0.5, `rgba(35,181,181,${0.18 - i * 0.06 + (speaking ? 0.06 : 0)})`);
  waveGradient.addColorStop(1, `rgba(35,181,181,${0.13 - i * 0.05 + (speaking ? 0.05 : 0)})`);
  ctx.fillStyle = waveGradient;

        ctx.beginPath();
        const waveCount = 6;
        const baseAmplitude = 8;
        const amplitude = baseAmplitude + (speaking ? volume * 60 : volume * 20);
        const phase = time * 0.015 + i * 0.01;

        for (let angle = 0; angle <= Math.PI * 2; angle += Math.PI / (waveCount * 15)) {
          const r =
            radius +
            Math.sin(angle * waveCount + phase) * amplitude * (0.9 - i * 0.25) +
            Math.cos(angle * waveCount * 0.6 + phase * 0.7) * (amplitude * 0.25);
  ...
  const ringGradient = ctx.createRadialGradient(cx, cy, radius * 0.6, cx, cy, radius * 1.1);
    ...
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80">
      ...
      <div className="flex items-center justify-center w-full h-full">
        <div
          ...
          style={{ width: '250px', height: '250px', boxShadow: '0 0 40px 0 #23b5b5' }}
        >
          <canvas
            ref={canvasRef}
            width={220}
